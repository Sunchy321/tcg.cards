...
- `site-console` 不再直接执行本地 auth handler，而是通过同源 `/auth/[...]` 路由转发到 `service-internal`

## 目标

1. 提供可独立运行和部署的 `service-internal` 登录服务
2. 抽离 `site-console` 与 `service-internal` 共用的服务端 auth 配置
3. 保持 `site-console` 浏览器侧仍然访问同源 `/auth/[...]`
...
2. 访问 `service-internal` 的 `/auth/*` 可处理登录相关请求
3. `site-console` 前端无需修改登录调用路径，仍然使用同源 `/auth/*`
