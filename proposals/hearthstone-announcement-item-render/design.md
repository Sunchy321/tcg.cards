...
glow 是 `renderModel` 的一个可选字段：`{ part, type: 'buff' | 'nerf' | 'rework' | 'neutral' }[]`。

- `buff`：该部位明确增强
- `nerf`：该部位明确削弱
- `rework`：该部位发生无法按增强或削弱归类的功能重做
- `neutral`：该部位发生不影响实际玩法的展示或措辞修改
...
公告渲染复用现有图片渲染流程的标准 `ImageRequirementRequest` 构建器。公告层只负责按 side 解析版本、合并 delta、为 curr 挂载 glow 和重新计算 renderHash。

- **预览**：调用渲染器并把 PNG 返回前端，不写 bucket，不写 `card_image_assets`
- **下载 PNG**：复用预览的无副作用渲染；指定语言时分别下载各 side 的 PNG，全部语言时把所有语言和 side 打包为 ZIP
- **复制/下载请求**：指定语言时，单图显示一个“复制请求”，双图显示“复制前图请求”和“复制后图请求”；全部语言时只显示一个“下载请求”，保存包含全部语言和 side 的 requirements JSON
- **写入存储**：独立调用渲染器，把 PNG 转换为 WebP，写入 bucket 并 upsert `card_image_assets`；不要求先预览

全部语言模式的界面预览固定展示 `zhs`；语言选择不改变已经生成的内存预览数据。
