...
import { Card as ICard, Legalities } from '@interface/yugioh/card';
...
export function getLegalityRules(): LegalityRule[] {
    ...
    const ruleList = internalData<string[]>('yugioh.legality.rule');

    for (const r of ruleList) {
        const yaml = internalData<RuleYAML<Legality>>(`yugioh.legality.rule.${r}`);
