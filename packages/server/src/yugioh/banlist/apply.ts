import Format from '@/yugioh/db/format';
import FormatAnnouncement from '@/yugioh/db/format-announcement';
import FormatChange from '@/yugioh/db/format-change';
import Set from '@/yugioh/db/set';

import { Document } from 'mongoose';

import { Format as IFormat } from '@interface/yugioh/format';
import {
    FormatAnnouncement as IFormatAnnouncement,
    FormatChange as IFormatChange,
    Legality,
} from '@interface/yugioh/format-change';
    ...
    async apply(): Promise<void> {
        ...
        const pseudoReleaseAnnouncement = this.sets.filter(
            s => !this.announcements.some(
                a => a.changes.some(
                    c => c.format === '#common' && c.setIn?.includes(s.id),
                ),
            ),
        ).map(s => ({
            source:  'release',
