import { Column, is, isNotNull, isNull, SQL } from 'drizzle-orm';
...
function query(options: CostQueryOption): SQL {
    const { column, costMapCol, parameter, operator, qualifier } = options;
...
export default function cost(command: CostCommand, options: CostServerOption): CostServerCommand {
    const { column, costMapCol } = options;

    return {
        ...command,
        query: args => query({ column, costMapCol, ...args }),
    };
}
