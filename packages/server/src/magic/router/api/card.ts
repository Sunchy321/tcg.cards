...
import CardTemp from '@/magic/db/card-temp';
...
import { Card as ICardTemp } from '@interface/magic/card-temp';
...
router.get('/', async ctx => {
    ...
    const aggregate = CardTemp.aggregate().allowDiskUse(true);
    ...
    const cards: ICardTemp[] = await aggregate;

    const versions = await CardTemp.aggregate<Version>()
        .match({ cardId: id })
        .sort({ releaseDate: -1 })
        .project({
            _id:         0,
...
router.get('/profile', async ctx => {
    ...
    const cards = await CardTemp.find({ cardId: { $in: ids } });
