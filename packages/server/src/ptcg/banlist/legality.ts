...
import { Card as ICard, Legalities } from '@interface/ptcg/card';
...
import { formats as formatList } from '@static/ptcg/basic';
...
export function getLegalityRules(): LegalityRule[] {
    ...
    const ruleList = internalData<string[]>('ptcg.legality.rule');

    for (const r of ruleList) {
        const yaml = internalData<RuleYAML<Legality>>(`ptcg.legality.rule.${r}`);
