...
import { Card as ICard, Legalities } from '@interface/hearthstone/card';
import { Format as IFormat } from '@interface/hearthstone/format';
import { Legality } from '@interface/hearthstone/format-change';
...
import { formats as formatList } from '@static/hearthstone/basic';
...
export function getLegalityRules(): LegalityRule[] {
    ...
    const ruleList = internalData<string[]>('hearthstone.legality.rule');

    for (const r of ruleList) {
        const yaml = internalData<RuleYAML<Legality>>(`hearthstone.legality.rule.${r}`);
