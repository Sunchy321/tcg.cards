...
/**
 * Field-level commit history for the collaborative overlay track.
 * Magic is the first game to actually use these tables (hearthstone defines
 * equivalents but never wires them in). Structure mirrors
 * `shared/hearthstone/field-sync.ts`.
 */
...
/** Per-field current winner source (auto vs manual overlay). */
...
/** Tracks the last pushed and pulled commit sequence for one local consumer. */
