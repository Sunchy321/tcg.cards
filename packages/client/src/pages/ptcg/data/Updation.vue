...
const loadData = async () => {
    const { data: result } = await controlGet<UpdationData>(`/ptcg/${mode.value}/get-updation`);
...
const commitUpdation = async (updation: Updation & { _id: string }, type: string) => {
    await controlPost(`/ptcg/${mode.value}/commit-updation`, {
        id:  updation._id,
        key: updation.key,
        type,
    });
...
const acceptAllUpdation = async () => {
    ...
    if (first != null) {
        await controlPost(`/ptcg/${mode.value}/accept-all-updation`, { key: first.key });
        await loadData();
    }
};
...
const rejectAllUpdation = async () => {
    ...
    if (first != null) {
        await controlPost(`/ptcg/${mode.value}/reject-all-updation`, { key: first.key });
        await loadData();
    }
};
...
const acceptUnchanged = async () => {
    ...
    if (first != null) {
        await controlPost(`/ptcg/${mode.value}/accept-unchanged`, { key: first.key });
        await loadData();
    }
};
