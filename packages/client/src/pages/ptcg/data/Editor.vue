...
const getLegality = async () => {
    const { data: result } = await controlGet('/ptcg/card/get-legality', {
        id: id.value,
    });
...
const extractRulingCards = async () => {
    const { data: cards } = await controlGet('/ptcg/card/extract-ruling-cards', {
        id: id.value,
    });
...
(window as any).extract = async (ids: string[]) => {
    await controlGet('/ptcg/card/extract-ruling-cards', {
        id: ids.join(','),
    });
};
...
const doUpdate = debounce(
    async () => {
        ...
        await controlPost('/ptcg/card/update', {
            data: card.value,
        });

        await controlPost('/ptcg/print/update', {
            data: print.value,
        });

        await controlPost('/ptcg/card/update-related', {
...
const loadData = async () => {
    if (id.value != null && lang.value != null && set.value != null && number.value != null) {
        const { data: result } = await controlGet<CardEditorView>('/ptcg/card/raw', {
            id:     id.value,
            lang:   lang.value,
            set:    set.value,
            number: number.value,
...
const loadGroup = async (method: string, skip = false) => {
    ...
    if (method.startsWith('search:')) {
        request = await controlGet<CardGroup>('/ptcg/card/search', {
            'q':         search.value,
            'sample':    sampleValue,
            'filter-by': filterBy.value,
        });
    } else {
        request = await controlGet<CardGroup>('/ptcg/card/need-edit', {
            method,
