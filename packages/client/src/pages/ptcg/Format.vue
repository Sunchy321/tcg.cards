<template>
    <q-page class="q-pa-md">
        <div class="row items-center q-mb-lg text-h5">
            <div class="q-mr-sm">{{ $t('ptcg.format.' + format) }}</div>
        ...
        <template v-else>
            <div class="flex items-center q-mb-md">
                <span class="text-h6">{{ $t('ptcg.ui.format.banlist') }}</span>
            ...
            <div v-if="sets.length > 0" class="flex q-my-md">
                <span class="text-h6">{{ $t('ptcg.ui.format.set') }}</span>
            </div>
...
useTitle(() => i18n.t('ptcg.format.$self'));
...
const format = useParam('format', {
    ...
    inTitle: true,
    values:  formats,
    label:   (v: string) => i18n.t(`ptcg.format.${v}`),
});
...
const loadData = async () => {
    const { data: formatResult } = await apiGet<Format>('/ptcg/format', {
        id: format.value,
    });

    data.value = formatResult;

    const { data: changesResult } = await apiGet<FormatChange[]>('/ptcg/format/changes', {
        id: format.value,
    });
