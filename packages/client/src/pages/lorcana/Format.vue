...
import { FormatChange, Legality } from '@model/lorcana/schema/game-change';
...
const format = useParam('format', {
    ...
    inTitle: true,
    values:  formats,
    label:   v => i18n.t(`lorcana.format.${v}`),
});
...
const loadData = async () => {
    data.value = await trpc.lorcana.format.full({ formatId: format.value });

    changes.value = await trpc.lorcana.format.changes({ formatId: format.value });
};
