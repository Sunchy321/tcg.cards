<template>
    <div class="q-pa-md">
        ...
        >
            <div>
                <span>{{ $t(`hearthstone.format.${w.format}`) }} {{ w.legality[0] }} {{ w.legality[1] }}</span>
                <q-btn
                    flat round dense
                    icon="mdi-close"
...
export default defineComponent({
    ...
    setup() {
        ...
        const assign = async () => {
            ...
            const ws = controlWs('/hearthstone/format/assign-legality');
        ...
        const test = async () => {
            ...
            const ws = controlWs('/hearthstone/format/test-legality');
            return new Promise((resolve, reject) => {
                ws.onmessage = ({ data }) => {
                    progress.value = JSON.parse(data) as Status;
                };
