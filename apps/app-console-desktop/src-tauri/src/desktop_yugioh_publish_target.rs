...
use crate::desktop_runtime_config_sync::{post_runtime_json, sync_desktop_runtime_config_once};
use crate::{
    load_desktop_config, store_desktop_config, trim_non_empty, StoredYugiohPublishTargetProfile,
    StoredYugiohSettings, CREDENTIAL_SERVICE,
};

const YUGIOH_PUBLISH_TARGET_ACCOUNT: &str = "yugioh-publish-target";

/// In-process cache for the Yugioh publish target connection string.
pub(crate) struct YugiohPublishTargetConnectionStringCache {
    current: Mutex<Option<Option<String>>>,
}

impl Default for YugiohPublishTargetConnectionStringCache {
...
pub(crate) struct DesktopYugiohPublishTargetSettings {
...
#[serde(rename_all = "camelCase")]
pub(crate) struct DesktopYugiohPublishTargetTestResult {
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct DesktopYugiohPublishTargetValidationResult {
    is_valid: bool,
    reasons: Vec<String>,
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ResolvedYugiohPublishTarget {
...
fn write_cached_connection_string(
    ...
    let cache = app.state::<YugiohPublishTargetConnectionStringCache>();
    let mut current = cache
        .current
        .lock()
        .map_err(|_| "Failed to lock Yugioh publish target connection string cache.".to_string())?;
...
pub(crate) fn load_publish_target_connection_string(
    ...
    let entry = keyring::Entry::new(CREDENTIAL_SERVICE, YUGIOH_PUBLISH_TARGET_ACCOUNT).map_err(
        |error| format!("Failed to open Yugioh publish target credential entry: {error}"),
    )?;
    ...
    match entry.get_password() {
        ...
        Err(error) => Err(format!(
            "Failed to read Yugioh publish target connection string: {error}"
        )),
    }
}
    ...
    match trim_non_empty(connection_string) {
        Some(connection_string) => {
            entry.set_password(&connection_string).map_err(|error| {
                format!("Failed to store Yugioh publish target connection string: {error}")
            })?;
        ...
        None => match entry.delete_credential() {
            ...
            Err(error) => Err(format!(
                "Failed to clear Yugioh publish target connection string: {error}"
            )),
        },
    }
}
...
fn build_publish_target_settings(
    profile: Option<&StoredYugiohPublishTargetProfile>,
    connection_string: Option<String>,
) -> DesktopYugiohPublishTargetSettings {
    DesktopYugiohPublishTargetSettings {
...
pub(crate) fn load_publish_target_profile(
    app: &AppHandle,
) -> Result<Option<StoredYugiohPublishTargetProfile>, String> {
...
fn load_publish_target_settings(
    app: &AppHandle,
) -> Result<DesktopYugiohPublishTargetSettings, String> {
...
fn resolve_requested_target_fields(
    ...
    let publish_target_id =
        publish_target_id.ok_or_else(|| "Yugioh publish target id is required.".to_string())?;
    let environment =
        environment.ok_or_else(|| "Yugioh publish target environment is required.".to_string())?;
    let connection_string = connection_string
        .ok_or_else(|| "Yugioh publish target connection string is required.".to_string())?;

    if environment != "test" {
        return Err("Yugioh publication is restricted to the test environment.".to_string());
    }
...
) -> Result<ResolvedYugiohPublishTarget, String> {
    post_runtime_json(
        "desktop/test-yugioh-publish-target",
        json!({
...
pub(crate) async fn require_resolved_publish_target(
    ...
    let profile = load_publish_target_profile(app)?
        .ok_or_else(|| "Yugioh publish target profile is not configured.".to_string())?;
    let connection_string = load_publish_target_connection_string(app)?
        .ok_or_else(|| "Yugioh publish target connection string is not configured.".to_string())?;
    ...
    if target.target_fingerprint != profile.target_fingerprint {
        return Err(format!(
            "Configured Yugioh publish target fingerprint changed: expected {}, current {}.",
...
#[tauri::command]
pub(crate) fn desktop_get_yugioh_publish_target(
    app: AppHandle,
) -> Result<DesktopYugiohPublishTargetSettings, String> {
...
#[tauri::command]
pub(crate) async fn desktop_test_yugioh_publish_target(
    publish_target_id: Option<String>,
    environment: Option<String>,
    connection_string: Option<String>,
) -> Result<DesktopYugiohPublishTargetTestResult, String> {
    let (publish_target_id, environment, connection_string) =
        resolve_requested_target_fields(publish_target_id, environment, connection_string)?
            .ok_or_else(|| "Yugioh publish target settings are required.".to_string())?;
    let started_at = Instant::now();
    let target = resolve_publish_target(publish_target_id, environment, connection_string).await?;

    Ok(DesktopYugiohPublishTargetTestResult {
...
#[tauri::command]
pub(crate) async fn desktop_set_yugioh_publish_target(
    app: AppHandle,
    publish_target_id: Option<String>,
    environment: Option<String>,
    connection_string: Option<String>,
    ...
    let Some((publish_target_id, environment, connection_string)) = requested else {
        ...
        sync_desktop_runtime_config_once(&app).await?;
        return Ok(DesktopYugiohPublishTargetSettings {
    ...
    store_publish_target_profile(
        &app,
        Some(StoredYugiohPublishTargetProfile {
            publish_target_id,
            environment,
    ...
    Ok(DesktopYugiohPublishTargetSettings {
...
#[tauri::command]
pub(crate) async fn desktop_validate_yugioh_publish_target_binding(
    app: AppHandle,
    publish_target_id: String,
    environment: String,
    target_fingerprint: String,
) -> Result<DesktopYugiohPublishTargetValidationResult, String> {
    let expected_publish_target_id = trim_non_empty(Some(publish_target_id))
        .ok_or_else(|| "Expected Yugioh publish target id is required.".to_string())?;
    let expected_environment = trim_non_empty(Some(environment))
        .ok_or_else(|| "Expected Yugioh publish target environment is required.".to_string())?;
    let expected_target_fingerprint = trim_non_empty(Some(target_fingerprint))
        .ok_or_else(|| "Expected Yugioh publish target fingerprint is required.".to_string())?;
    ...
    Ok(DesktopYugiohPublishTargetValidationResult {
