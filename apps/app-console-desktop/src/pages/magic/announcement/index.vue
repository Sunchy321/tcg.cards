...
function toggleYamlMode() {
  if (isYamlMode.value) {
    if (fromYaml(yamlContent.value)) {
      ...
      toast.add({ title: 'YAML 格式错误', description: yamlError.value, color: 'error' });
    }
  } else {
...
async function selectAnnouncement(item: AnnouncementProfile) {
  ...
  try {
    const detail = await orpc.magic.announcement.get({ id: item.id });
...
async function loadAnnouncements() {
  ...
  try {
    ...
    toast.add({ title: '加载失败', description: error instanceof Error ? error.message : '请稍后重试', color: 'error' });
  } finally {
    loading.value = false;
  }
}
...
async function handleSubmit() {
  ...
  try {
    const payload = {
      ...
      name:          form.name,
      effectiveDate: form.effectiveDate || null,
      link:          form.link.filter(l => l.trim() !== ''),
    };
    ...
    if (form.id) {
      ...
      const result = await orpc.magic.announcement.create(payload);
      if (result) { selectedId.value = result.id; form.id = result.id; isCreating.value = false; }
      toast.add({ title: '创建成功', color: 'success' });
    }
    await loadAnnouncements();
  } catch (error) {
    toast.add({ title: '保存失败', description: error instanceof Error ? error.message : '请稍后重试', color: 'error' });
  } finally {
    saving.value = false;
  }
}
...
async function handleDelete() {
  ...
  try {
    ...
    toast.add({ title: '删除失败', description: error instanceof Error ? error.message : '请稍后重试', color: 'error' });
  } finally {
