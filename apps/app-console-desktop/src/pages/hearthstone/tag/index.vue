    ...
    <div class="rounded-xl border border-slate-200 bg-white p-4">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-muted">查询和编辑本地 hearthstone.tags 中的解析、规范化与字段投影配置。</p>
        </div>
        <div class="flex gap-2">
          <UButton label="刷新" icon="i-lucide-rotate-ccw" color="neutral" variant="ghost" :loading="loading" @click="loadTags" />
        </div>
      </div>
    </div>
...
async function loadTags() {
  ...
  try {
    ...
    toast.add({ title: '加载失败', description: error instanceof Error ? error.message : String(error), color: 'error' });
  } finally {
    loading.value = false;
  }
}
...
async function selectTag(enumId: number) {
  ...
  try {
    const tag = await client.tag.get({ enumId });
    applyTag(tag);
  } catch (error) {
    toast.add({ title: '加载详情失败', description: error instanceof Error ? error.message : String(error), color: 'error' });
  } finally {
