...
type LabelKind = 'class' | 'type' | 'race' | 'spellSchool' | 'relation';
...
export function getMetadataLabel(kind: LabelKind, value: string, lang: Locale) {
  return labels[kind][lang]?.[value] ?? labels[kind].en?.[value] ?? value;
}

export function pickLabels(lang: Locale) {
  const result: Partial<Record<LabelKind, Record<string, string>>> = {};
  for (const kind of Object.keys(labels) as LabelKind[]) {
    result[kind] = labels[kind][lang] ?? labels[kind].en ?? {};
  }
  return result;
}
