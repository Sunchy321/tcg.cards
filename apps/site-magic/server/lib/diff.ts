...
// Encode special sequences (e.g. "{W}", "{T}") into private-use unicode chars
// so that diff operates on them as atomic units.
...
function decode(text: string, map: Record<string, string>): string {
  return [...text].map(p => {
    if (p.codePointAt(0)! >= 0xF0000) {
      return Object.entries(map).find(([, v]) => v === p)?.[0] ?? p;
    }
    return p;
  }).join('');
}
  ...
  for (const d of diffWordsWithSpace(lhsEncoded, rhsEncoded)) {
    if (d.added) {
      ...
      if (lastDiff == null || typeof lastDiff === 'string') {
        ...
        if (typeof lastDiff === 'string') {
          diffs[diffs.length - 1] = (lastDiff as string) + d.value;
        }
      } else if (d.value === ' ') {
        lastDiff[0] += d.value;
        lastDiff[1] += d.value;
  ...
  while (prevDiff.length > 0 && nextDiff.length > 0) {
    ...
    const type = (() => {
      if (typeof lastPrev === 'string') {
        return typeof lastNext === 'string' ? 'common' : 'remove';
      } else {
        return typeof lastNext === 'string' ? 'add' : 'dual';
      }
    })();
