      ...
      <template #header>
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-base font-semibold text-gray-900 dark:text-white">
            {{ $t(`magic.dataSources.sections.${section.id}.title`) }}
          </h2>
        </div>
      </template>
      ...
      <div class="flex flex-col gap-5">
        <div v-for="group in section.groups" :key="group.id" class="flex flex-col gap-3">
          <h3 v-if="group.heading" class="text-sm font-semibold text-gray-900 dark:text-white">
            {{ $t(`magic.dataSources.sections.${section.id}.groups.${group.id}.title`) }}
          </h3>
          ...
          >
            {{ $t(`magic.dataSources.sections.${section.id}.groups.${group.id}.${text}`) }}
          </p>
          ...
          <ul v-if="group.sources" class="flex flex-col gap-3">
            <li v-for="source in group.sources" :key="source.id" class="flex items-start gap-3">
              ...
              <p class="text-sm leading-relaxed min-w-0">
                ...
                >
                  {{ $t(`magic.dataSources.sources.${source.id}.name`) }}
                </NuxtLink>
                <span v-else class="font-medium text-gray-900 dark:text-white">
                  {{ $t(`magic.dataSources.sources.${source.id}.name`) }}
                </span>
                <span class="text-gray-600 dark:text-gray-400">
                  — {{ $t(`magic.dataSources.sources.${source.id}.desc`) }}
                </span>
              </p>
            </li>
          </ul>
        ...
        <div v-if="section.legalLinks" class="flex flex-wrap gap-x-4 gap-y-1 pt-1">
          ...
          >
            ...
            {{ $t(`magic.dataSources.legal.${doc.key}`) }}
          </NuxtLink>
        </div>
      </div>
    </UCard>
...
useTitle(() => t('magic.dataSources.$self'));
...
const sections: Section[] = [
    ...
    groups: [
      {
        ...
        texts:   ['body'],
        sources: [
          { id: 'scryfall', img: '/scryfall.svg', imgClass: 'dark:invert', href: 'https://scryfall.com' },
        ],
      },
      {
        id:      'text',
        ...
        texts:   ['body'],
        sources: [
          { id: 'mtgch', img: '/mtgch.svg', imgClass: 'dark:invert', href: 'https://mtgch.com' },
        ],
      },
    ],
  },
    ...
    groups: [
      {
        ...
        texts:   ['body'],
        sources: [
          { id: 'scryfallImages', img: '/scryfall.svg', imgClass: 'dark:invert', href: 'https://scryfall.com' },
          { id: 'gatherer', img: '/gatherer.svg', href: 'https://gatherer.wizards.com' },
          { id: 'community', icon: 'i-lucide-image-plus' },
        ],
      },
    ],
  },
    ...
    groups: [
      {
        ...
        texts:   [],
        sources: [
          { id: 'mtgjson', img: '/mtgjson.png', href: 'https://mtgjson.com' },
        ],
      },
    ],
  },
  {
    ...
    groups: [
      ...
      { id: 'attribution', heading: true, texts: ['fanPolicy', 'credits'] },
    ],
    legalLinks: true,
  },
];
