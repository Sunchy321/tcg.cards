  ...
  <div class="space-y-6">
    ...
    <UCard>
      ...
      >
        ...
        <template #dataStatus-cell="{ getValue }">
          <UBadge
            :color="getDataStatus(getValue() as RuleVersion['dataStatus']).color"
            variant="subtle"
            size="sm"
          >
        ...
        <template #actions-cell="{ row }">
          <div class="flex items-center gap-2">
            <UButton
              v-if="row.original.dataStatus === 'pending'"
              icon="i-lucide-play"
              variant="ghost"
              size="xs"
              color="primary"
            ...
            <UModal v-else-if="row.original.dataStatus === 'imported'" title="重新导入">
              <UButton
              ...
              <template #body>
                <p class="text-sm">
                  确定要重新导入规则版本 <strong>{{ row.original.id }}</strong> 吗？<br>
                  这会基于 R2 中的当前 TXT 文件重建该版本的节点数据。
                </p>
              </template>
              ...
              <template #footer="{ close }">
                <div class="flex justify-end gap-2">
                  <UButton
                    color="warning"
                    :loading="loadingId === row.original.id"
                    @click="reimportRule(row.original, close)"
                  >
                    确认重新导入
                  </UButton>
                </div>
...
function getDataStatus(value: RuleVersion['dataStatus']) {
  if (value === 'imported') {
...
async function reimportRule(version: { id: string }, close: () => void) {
  const success = await loadRule(version);
  if (success) {
    close();
  }
}
