...
/** Runtime-local Drizzle database shape used by Shadowverse procedures. */
export type ShadowverseLocalDb = ReturnType<typeof createDb>;

/** Cached local database client paired with its active connection string. */
interface ShadowverseLocalDbState {
  connectionString: string;
  db: ShadowverseLocalDb;
}

let localDbState: ShadowverseLocalDbState | null = null;

/** Closes a replaced local client after the configured connection changes. */
function disposeLocalDb(db: ShadowverseLocalDb) {
...
export function getShadowverseLocalDb() {
  ...
  } satisfies ShadowverseLocalDbState;
