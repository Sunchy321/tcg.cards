...
export class DefinitionInitBuilder {
  ...
  scope<TS extends z.ZodTypeAny>(schema: TS, config: ScopeConfig<z.infer<TS>>): DefinitionBuilderScope<z.infer<TS>> {
    return new DefinitionBuilderScope(this.taskType, this.version, this.supportsResume, this.effectModel, schema as z.ZodType<z.infer<TS>>, config);
  }
}
...
export class DefinitionBuilderScope<TScope> {
  ...
  input<TI extends z.ZodTypeAny>(schema: TI): DefinitionBuilderInput<TScope, z.infer<TI>> {
    return new DefinitionBuilderInput(this.taskType, this.version, this.supportsResume, this.effectModel, this.scopeSchema, this.scopeConfig, schema as z.ZodType<z.infer<TI>>);
  }
}
...
export class DefinitionBuilderInput<TScope, TInput> {
  ...
  output<O extends z.ZodTypeAny>(schema: O): DefinitionBuilderOutput<TScope, TInput, z.infer<O>> {
    return new DefinitionBuilderOutput<TScope, TInput, z.infer<O>>(this.taskType, this.version, this.supportsResume, this.effectModel, this.scopeSchema, this.scopeConfig, this.inputSchema, schema as z.ZodType<z.infer<O>>);
  }
}
...
export class DefinitionBuilderOutput<TScope, TInput, TOutput> {
  ...
  context<TCtx>(decl: ContextDeclaration<TCtx, TInput, TScope>): DefinitionBuilder<TScope, TInput, TOutput, TCtx, TInput> {
    return new DefinitionBuilder(
      this.taskType, this.version, this.supportsResume, this.effectModel,
...
export class StageBuilder<TScope, TInput, TOutput, TCtx, TPrevOutput, TStageKey extends string> {
  ...
  entry<TBlockInput>(fn: (args: { scope: TScope; ctx: TCtx; input: TPrevOutput }) => { total?: number; blockInput: TBlockInput } | Promise<{ total?: number; blockInput: TBlockInput }>): ChunkedStageBuilder<TScope, TInput, TOutput, TCtx, TPrevOutput, TStageKey, TBlockInput> {
...
export class ChunkedStageBuilder<TScope, TInput, TOutput, TCtx, TPrevOutput, TStageKey extends string, TBlockInput> {
  ...
  constructor(
    ...
    readonly stageKey: TStageKey,
    readonly stageMeta: StageMeta,
    readonly entryFn: (args: { scope: TScope; ctx: TCtx; input: TPrevOutput }) => { total?: number; blockInput: TBlockInput } | Promise<{ total?: number; blockInput: TBlockInput }>,
    enableConfig?: { when: (input: TInput) => boolean; otherwise: (input: TInput) => unknown },
    readonly blockFn?: (args: { scope: TScope; ctx: TCtx; blockInput: TBlockInput; progress: ProgressFn<any>; done: (finalBlockInput: TBlockInput) => BlockDone }) => TBlockInput | BlockDone | Promise<TBlockInput | BlockDone>,
  ) {
    this.enableConfig = enableConfig;
  }
  ...
  block(fn: (args: { scope: TScope; ctx: TCtx; blockInput: TBlockInput; progress: ProgressFn<any>; done: (finalBlockInput: TBlockInput) => BlockDone }) => TBlockInput | BlockDone | Promise<TBlockInput | BlockDone>): ChunkedStageBuilder<TScope, TInput, TOutput, TCtx, TPrevOutput, TStageKey, TBlockInput> {
...
interface PerRunState {
  ...
  blockFn: ((args: { scope: unknown; ctx: unknown; blockInput: unknown; progress: (...args: any[]) => void; done: (finalBlockInput: unknown) => BlockDone }) => unknown | BlockDone | Promise<unknown | BlockDone>) | null;
...
function buildAnyTaskDefinition(builder: DefinitionBuilder<any, any, any, any, any>): AnyTaskDefinition {
  const { taskType, version, supportsResume, effectModel, scopeSchema, scopeConfig, inputSchema, outputSchema, contextDecl, stages } = builder;

  const scopeType = scopeConfig.type;

  function inputFromRun(run: TaskRunInput): unknown { return run.params; }
  function scopeFromRun(run: TaskRunInput): unknown { return run.scope.snapshot ?? {}; }
  ...
  return {
    ...
    resolveScope(input: unknown) {
      ...
      return scopeConfig.resolve(rawScope);
    },
    ...
    async prepareStageEntry(args) {
      ...
      const entryResult = await st.hooks.entry({ scope: scopeFromRun(run), ctx: state.ctx, input: entryInput });
    ...
    async executeBlock(args) {
      ...
      const done = (finalBlockInput: unknown) => { state.exitBlockInput = finalBlockInput; return mkDone(); };
      const result = await state.blockFn!({ scope: undefined, ctx: state.ctx, blockInput, progress, done });
...
function getEntryInput(
  allStages: { key: string; kind: string }[],
  currentStage: { key: string; kind: string },
  state: PerRunState,
): unknown {
  const idx = allStages.findIndex(s => s.key === currentStage.key);
  return idx === 0 ? null : state.previousStageOutput;
}
...
function mkProgress(
  ...
  return (update?: any) => {
    if (update == null) { store.updateStage(taskRunId, stageKey, {}).catch(() => {}); return; }
...
export function createDefinition(taskType: string, config: { version: string; effectModel?: TaskEffectModel }): DefinitionInitBuilder {
  return new DefinitionInitBuilder(taskType, config.version, undefined, config.effectModel);
}
