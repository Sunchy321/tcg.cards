...
- 当前计划接受 `sourceSpan` 直接放入 `raw_entity_snapshots`、`entities`、`entity_localizations`
- 当前计划接受 `renderHash` 由 canonical `renderModel` 生成，而不是使用随机值

### 必须确认

- 实施前确认 `int4multirange` 在 Drizzle 中的类型封装方式
- 实施前冻结首批渲染字段白名单
- 实施前补齐首批 `Tag -> 字段` 映射清单
...
- `entities`：结构修订 + 版本范围
- `entity_localizations`：语言文本 + 渲染模型 + 版本范围
...
设计里提出了 `int4multirange`，这是模型上最漂亮的方案，但实现复杂度与 ORM 支持要尽早验证。如果这一点拖到开发中途再改，会影响：
